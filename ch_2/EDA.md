...
(от 21:44 3-11-2024) https://colab.research.google.com/drive/1Im7gdpjKY5fOQ19gPQb5OnVdz3t6v0xc#scrollTo=Zjv7dgMJrHTR

## Выводы
# Анализ Данных Акций

## 1. Общее количество данных и структура
- В наборе данных содержится **63,082 записи**, что указывает на значительный объем информации.
- Данные включают следующие колонки: **TRADEDATE, OPEN, HIGH, LOW, CLOSE, VOLUME, TICKER**. Это позволяет проводить всесторонний анализ временных рядов по акциям.

## 2. Пропуски в данных
### Общее количество пропусков:
- В столбце **TRADEDATE** — 213 пропусков (33.76%). Связано с недавним размещением YDEX на MOEX.
- В столбцах **OPEN, HIGH, LOW, и CLOSE** по 1655 пропусков в каждом (по 2.62%).

### Вывод:
- Высокий процент пропусков в дате (TRADEDATE) критически важен для временного анализа, так как без дат невозможно корректно агрегировать данные. Пропуски в ценах также могут повлиять на статистические вычисления и модели.
...
- Коэффициенты корреляции показывают, что:
  ...
  - **GMKN** и **NVTK** имеют высокую корреляцию с другими тикерами, что может свидетельствовать о том, что их поведение связано с общими рыночными факторами.

## 7. Общие Выводы
- **Необходимость дополнительного анализа**: Высокая волатильность некоторых акций требует применения более продвинутых методов анализа временных рядов, чтобы учесть выбросы и колебания. Хотя волатильность акций ГМКН прежде всего связана с их дроблением акций, поэтому проблема решается нормализацией данных.
...
- **Анализ взаимосвязей**: Для глубокого понимания динамики рынка будет полезно в будущем провести анализ корреляции между акциями и сравнить результаты с макроэкономическими показателями.

## 8. Выводы по трендам акций первого эшелона
...
- Результаты анализа подчеркивают важность мониторинга волатильности и ликвидности акций, а также необходимость учета выбросов и зависимостей при планировании инвестиционных стратегий.

## 9. Выводы по сезонности
...
## 10. Выводы о наличии выбросов для различных акций
...
- Тикеры с выбросами (особенно **LKOH, YDEX, SNGS и NLMK**) заслуживают дополнительного внимания, так как колебания их цен могут отражать краткосрочные рыночные ожидания или чувствительность к внешним событиям. Эти компании могут быть более волатильными, что может представлять как риски, так и возможности для инвесторов.
- Тикеры с более высокой волатильностью (например, **PLZL**) могут быть интересны для активных трейдеров. Напротив, стабильные тикеры могут подойти для более консервативных стратегий инвестирования.
