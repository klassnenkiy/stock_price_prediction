...
def knn_rolling_with_backtest_improved(ticker_data, window_size=20, test_forecast_days=30, future_forecast_days=30, noise_scale=1.0):
    ...
    plt.title(f'k-NN Rolling Forecast (Improved) for {ticker_data["TICKER"].iloc[0]}\n'
...
knn_improved_results = {}
for ticker in first_tier_tickers:
    ticker_data = data[data['TICKER'] == ticker]
    result = knn_rolling_with_backtest_improved(ticker_data, window_size=20, test_forecast_days=30, future_forecast_days=30, noise_scale=1.0)
    if result is not None:
        knn_improved_results[ticker] = result
