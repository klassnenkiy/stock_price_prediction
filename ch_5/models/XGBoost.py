...
def xgboost_rolling_with_backtest_improved(ticker_data, window_size=20, test_forecast_days=30, future_forecast_days=30, noise_scale=1.0):
    ...
    plt.title(f'XGBoost Rolling Forecast (Improved) for {ticker_data["TICKER"].iloc[0]}\n'
...
for ticker in first_tier_tickers:
    ...
    result = xgboost_rolling_with_backtest_improved(ticker_data, window_size=20, test_forecast_days=30, future_forecast_days=30, noise_scale=1.0)
    if result is not None:
        xgb_improved_results[ticker] = result
